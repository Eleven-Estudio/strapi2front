...
| `--clean` | Auto-clean orphaned files without prompting |
...
This appears when switching between `by-feature` and `by-layer` structures. Use `--clean` to remove old files automatically.
