...
**Type:** `"by-feature" | "by-layer"`
**Default:** `"by-layer"`

Organization structure:
- `by-feature`: Groups files by content type (recommended)
- `by-layer`: Groups files by type (types/, services/, etc.)
...
Subdirectory for types (only used with `by-layer` structure).
...
Subdirectory for services (only used with `by-layer` structure).

#### `output.actions`

**Type:** `string`
**Default:** `"actions/strapi"`

Subdirectory for Astro actions (only used with `by-layer` structure).
