...
export async function generateActions(
  ...
  const { outputDir, servicesImportPath } = options;
  ...
  for (const collection of schema.collections) {
    ...
    const content = generateCollectionActions(collection, servicesImportPath);
...
export const ${actionsName} = {
  ...
  /**
   * Get a single ${collection.singularName} by documentId
   */
  getOne: defineAction({
    input: z.object({
    ...
    handler: async ({ documentId, populate }) => {
      try {
        const result = await ${serviceName}.findOne(documentId, { populate });
...
` : ''}
  ...
  update: defineAction({
    ...
    handler: async ({ documentId, data }) => {
      try {
        const result = await ${serviceName}.update(documentId, data);
        return result;
      } catch (error) {
        throw new ActionError({
          code: 'INTERNAL_SERVER_ERROR',
  ...
  delete: defineAction({
    ...
    handler: async ({ documentId }) => {
      try {
        await ${serviceName}.delete(documentId);
        return { success: true };
      } catch (error) {
        throw new ActionError({
          code: 'INTERNAL_SERVER_ERROR',
