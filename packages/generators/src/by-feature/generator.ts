...
export async function generateByFeature(
  ...
  const { outputDir, features } = options;
  ...
  await writeFile(clientPath, await formatCode(generateClient()));
  ...
  for (const collection of schema.collections) {
    ...
    if (features.services) {
      ...
      await writeFile(servicePath, await formatCode(generateCollectionService(collection)));
      generatedFiles.push(servicePath);
    }

    if (features.actions) {
      const actionsPath = path.join(featureDir, 'actions.ts');
      await writeFile(actionsPath, await formatCode(generateCollectionActions(collection)));
  ...
  for (const single of schema.singles) {
    ...
    if (features.services) {
      const servicePath = path.join(featureDir, 'service.ts');
      await writeFile(servicePath, await formatCode(generateSingleService(single)));
      generatedFiles.push(servicePath);
    }
  }
...
function generateCollectionService(collection: CollectionType): string {
  const typeName = toPascalCase(collection.singularName);
...
function generateSingleService(single: SingleType): string {
  const typeName = toPascalCase(single.singularName);
...
function generateCollectionActions(collection: CollectionType): string {
  const typeName = toPascalCase(collection.singularName);
  const serviceName = toCamelCase(collection.singularName) + 'Service';
...
export const ${actionPrefix}Actions = {
  ...
  getOne: defineAction({
    ...
    handler: async (input) => {
      const data = await ${serviceName}.findOne(input.documentId);
      return { data };
    },
  }),
  ...
  update: defineAction({
    ...
    handler: async (input) => {
      const data = await ${serviceName}.update(input.documentId, input.data as Partial<${typeName}>);
      return { data };
    },
  }),
  ...
  delete: defineAction({
    ...
    handler: async (input) => {
      await ${serviceName}.delete(input.documentId);
      return { success: true };
    },
  }),
};
